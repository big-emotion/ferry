...
// src/lib/dispatch/ci-gate-action.ts
...
// src/lib/dispatch/ci-gate-action.ts
var logger = createLogger2("", "ferry:ci-gate");
function mapCheckRunsToStatus(runs) {
  if (runs.length === 0) return "pending";
  if (runs.some((r) => r.status !== "completed")) return "pending";
  const failing = /* @__PURE__ */ new Set(["failure", "timed_out", "cancelled", "action_required", "stale"]);
...
async function resolveChangesTransitionId(ticketKey) {
  const explicit = process.env.FERRY_ITER_TRANSITION_ID?.trim();
  if (explicit) return explicit;
  if (!process.env.FERRY_JIRA_BASE_URL || !process.env.FERRY_JIRA_EMAIL || !process.env.FERRY_JIRA_API_TOKEN) {
    return "";
  }
  try {
...
async function runCiGateAction() {
  ...
  if (status === "red") {
    const failed = checks.check_runs.filter(
      (r) => r.conclusion !== null && r.conclusion !== "success" && r.conclusion !== "neutral" && r.conclusion !== "skipped"
...
var invokedDirectly = typeof process !== "undefined" && process.argv[1]?.endsWith("ci-gate-action.js");
if (invokedDirectly) {
  void runCiGateAction().catch((err) => {
