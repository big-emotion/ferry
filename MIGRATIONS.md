...
- **(info)** **Branch-protection caveat — label approval vs. required PR reviews.** The Reviewer triggers the Merger via a `ferry-merge` `repository_dispatch` and sets the `ferry:approved` label on the PR, but does **not** post a formal GitHub PR review approval. If your default branch is protected with a **"Require a pull request before merging — Require approvals"** rule, `gh pr merge` will be rejected because no formal review approval has been posted. See `docs/CONFIGURATION.md` → Merger agent (FR32) → Branch-protection caveat for options. Consumers who do not enable the Merger are unaffected.
