...
This report's "FR coverage" matrix counts a requirement as covered when a story file references it and when the corresponding library module exists with passing unit tests. It does **not** verify that the runtime entry points (`src/agents/*/index.ts`) and workflow steps (`.github/workflows/*.yml`) actually invoke that library code.

After 9 epics declared `done`, runtime inspection (2026-04-28) found:

- All four agent workflows still run `echo "<role> placeholder"` instead of invoking agent code (`refine.yml:50`, `dev.yml:34`, `review.yml:34`, `iterate.yml:34`).
- All four `src/agents/*/index.ts` files are empty `export {}` stubs.
- `src/lib/io/jira.ts:36-37` and `src/lib/io/github.ts:39-40` are scaffolds with `// TODO: replace with real API call`.
- No `callLlm` HTTP implementation exists; only model-routing config.
- `mapError` (`src/lib/error-taxonomy/index.ts`) has zero callers in `src/agents/` or `src/lib/audit/`.
- `start_ms: ${{ github.run_id }}` in 5 workflow files passes a numeric ID rather than epoch ms (corrupts every audit row's `duration_ms`).
- `reconciler.yml:37` passes `run_id: ${{ github.run_id }}` instead of a ULID.

**Recommendation for future readiness checks:** add a **runtime-coverage gate** that, for each FR claiming coverage, traces from the workflow YAML through the agent entry-point down to the relevant library function and asserts the path is unbroken. Concretely: a script that parses each `.github/workflows/*.yml`, finds the agent step, asserts it invokes a `src/agents/*/index.ts` (not `echo`), then statically checks that the entry-point imports the library functions claimed by each FR's story file.

This gap is addressed by Epic 10 (`epics.md`) and tracked in `sprint-change-proposal-2026-04-28.md`.
