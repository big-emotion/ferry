...
The operator can move a Jira ticket through Ferry phases and observe real agent behaviour: sub-tasks created, branches and PRs opened, findings posted, iterations applied. Closes the runtime gap between the library modules delivered in Epics 1–9 and a deployable pilot.

**Context:** Added 2026-04-28 via `sprint-change-proposal-2026-04-28.md`. Epics 1–9 shipped library modules with passing unit tests but no integration: agent workflows ran `echo "<role> placeholder"`, agent entry-points were empty stubs, and the Jira / GitHub / LLM HTTP clients were scaffolds with `TODO: replace with real API call`. This epic builds the integration layer.

**FRs covered:** Realises (does not add) FR1–FR60 at the runtime level. Specifically wires FR8–FR12 (Refiner), FR14–FR18 (Developer), FR19–FR24, FR57–FR58 (Reviewer), FR25–FR29, FR59 (Iterator), FR41 (audit `duration_ms` correctness), FR47 (Jira write secret-scan), FR50–FR51 (reconciler ULID).

**Stories:**

| ID | Title | Primary file(s) |
| --- | --- | --- |
| 10-1 | Real Jira REST client | `src/lib/io/jira-rest.ts` (NEW), replace scaffold in `src/lib/io/jira.ts` |
| 10-2 | Real LLM HTTP client | `src/lib/llm/call.ts` (NEW) |
| 10-3 | Real GitHub REST client extension | `src/lib/io/github.ts` (replace scaffold) |
| 10-4 | Error-taxonomy formatter & sanitiser | `src/lib/error-taxonomy/format.ts` (NEW), fix `index.ts:11` |
...
| 10-9 | Workflow YAML wiring + start_ms + reconciler ULID | `.github/workflows/{refine,dev,review,iterate,reconciler}.yml` |
| 10-10 | Pre-bundled audit composite action | `dist/audit-emit.cjs`, `scripts/build-audit-bundle.ts`, `.github/actions/ferry-emit-audit/action.yml` |
| 10-11 | End-to-end smoke runbook + audit-Issue invariant doc | `docs/e2e-smoke.md`, `docs/operations.md` |

**Sequencing:** 10-1, 10-2, 10-3, 10-4, 10-10 land in parallel (no inter-dependencies). Then 10-5 → 10-6 → 10-7 → 10-8 in pipeline order, with 10-9 workflow blocks landing alongside each agent. 10-11 last.

**Acceptance:** End-to-end smoke run produces ≥4 real audit rows in the `ferry-audit` Issue with monotonic ULIDs, all `duration_ms < 24h`, all `outcome` values from the typed taxonomy. Re-running `bmad-check-implementation-readiness` (with the new runtime-coverage gate per the readiness-report addendum) reports zero gaps.
