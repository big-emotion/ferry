...
describe('concurrency group-key expression', () => {
  it.each(DISPATCH_WORKFLOWS)(
    '%s must use startsWith guard to prevent adversarial ticket_key injection',
    (name) => {
      const content = readWorkflow(name);
  ...
  it.each(DISPATCH_WORKFLOWS)('%s must route malformed payloads to sinkhole group', (name) => {
    const content = readWorkflow(name);
