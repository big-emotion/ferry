...
async function fetchAllPrFiles(octokit, owner, repo, prNumber) {
...
async function getFileContent(octokit, owner, repo, filename, ref) {
  try {
    ...
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      if (msg.role === "user" && Array.isArray(msg.content)) {
        ...
        if (content.some((b) => b.type === "tool_result")) {
          ...
          const { cache_control: _cc, ...rest } = last;
          content[content.length - 1] = rest;
          break;
        }
      }
    }
