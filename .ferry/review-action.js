...
function parseReviewOutput(content) {
...
**CI failed:** ${ciMessage}`
  ...
  const diffText = files.map((f) => `### ${f.filename} (+${f.additions} -${f.deletions})
...
${description}`
  ...
  const rawContent = response.content.find((b) => b.type === "text")?.text ?? "";
...
${issueLines || "_(no specific issues listed)_"}`;
  ...
  appendOutput({ input_tokens: inputTokens, output_tokens: outputTokens });
}
...
main().catch((err) => {
  console.error("[ferry:review-action] fatal:", err.message);
  process.exit(1);
});
