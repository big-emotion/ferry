...
async function emitAudit(payload, opts) {
  ...
  const { ticket: ticket2, phase: phase2, runId: runId2, model: model2, outcome: outcome2, usage, start: start2 } = payload;
  ...
  const auditLine = {
    ...
    outcome: outcome2,
    duration_ms: Math.round(Date.now() - start2),
...
var auditIssueStr = requireEnv("FERRY_AUDIT_ISSUE");
var owner = requireEnv("FERRY_OWNER");
var repo = requireEnv("FERRY_REPO");
var runId = requireEnv("FERRY_RUN_ID");
var ticket = requireEnv("FERRY_TICKET");
var phase = requireEnv("FERRY_PHASE");
var model = requireEnv("FERRY_MODEL");
var outcome = requireEnv("FERRY_OUTCOME");
var startMsStr = requireEnv("FERRY_START_MS");
var auditIssue = parseInt(auditIssueStr, 10);
if (isNaN(auditIssue)) throw new Error(`FERRY_AUDIT_ISSUE must be a number, got: ${auditIssueStr}`);
var start = parseInt(startMsStr, 10);
if (isNaN(start)) throw new Error(`FERRY_START_MS must be a number, got: ${startMsStr}`);
var inputTokens = parseInt(process.env["FERRY_INPUT_TOKENS"] ?? "0", 10) || 0;
