...
- Consumers can use `uses: big-emotion/ferry/.github/actions/ferry-run-developer@v1.0.1` with no build step in their own workflows. The action is ready to execute immediately on checkout.
- Version pinning works as expected: tagging a release freezes both the TypeScript source and its compiled output, so `@v1.0.1` is reproducible.
