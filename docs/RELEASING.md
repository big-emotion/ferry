...
| `v0.8.0` | Immutable | Never                                  | Default reference in consumer workflow stubs (composite action pins, e.g. `uses: big-emotion/ferry/.github/actions/ferry-run-refiner@v0.8.0`); recommended for production |
...
**Consumer recommendation:** Stay on the pinned `@v0.8.0` tag (or a SHA pin) for production. The floating `@v1` tag is available for consumers who prefer automatic minor/patch upgrades — see the SHA pinning section of the README.
...
Replace `@v0.8.0` with the exact commit SHA the tag points to:

```bash
LATEST_SHA=$(gh api repos/big-emotion/ferry/git/refs/tags/v0.8.0 --jq '.object.sha')
sed -i.bak "s|@v0.8.0|@${LATEST_SHA}|g" .github/workflows/ferry-*.yml
