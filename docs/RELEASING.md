...
| `v0.5.3` | Immutable | Never                                  | Default reference in consumer workflow stubs (`uses: big-emotion/ferry/.github/workflows/...@v0.5.3`); recommended for production |
...
**Consumer recommendation:** Stay on the pinned `@v0.5.3` tag (or a SHA pin) for production. The floating `@v1` tag is available for consumers who prefer automatic minor/patch upgrades — see the SHA pinning section of the README.
...
Replace `@v0.5.3` with the exact commit SHA the tag points to:

```bash
LATEST_SHA=$(gh api repos/big-emotion/ferry/git/refs/tags/v0.5.3 --jq '.object.sha')
sed -i.bak "s|@v0.5.3|@${LATEST_SHA}|g" .github/workflows/ferry-*.yml
