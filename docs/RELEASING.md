...
| `v1.1.0` | Immutable | Never                                  | Default reference in consumer workflow stubs (composite action pins, e.g. `uses: big-emotion/ferry/.github/actions/ferry-run-refiner@v1.1.0`); recommended for production |
...
**Consumer recommendation:** Stay on the pinned `@v1.1.0` tag (or a SHA pin) for production. The floating `@v1` tag is available for consumers who prefer automatic minor/patch upgrades — see the SHA pinning section of the README.
...
Replace `@v1.1.0` with the exact commit SHA the tag points to:

```bash
LATEST_SHA=$(gh api repos/big-emotion/ferry/git/refs/tags/v1.1.0 --jq '.object.sha')
sed -i.bak "s|@v1.1.0|@${LATEST_SHA}|g" .github/workflows/ferry-*.yml
