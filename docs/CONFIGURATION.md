...
- name: Run Developer agent
  id: run-developer
  uses: big-emotion/ferry/.github/actions/ferry-run-developer@v1.2.0
  with:
...
workflow:
  agents:
    ...
    merger:
      trigger_column: 'Ready to Merge' # moving a ticket here is an explicit merge order (ADR-0005 rev. 2)
...
| `workflow.agents.merger.trigger_column`            | `"Ready to Merge"`    | Jira column that triggers the Merger (ADR-0005 rev. 2). Moving a ticket into this column is an explicit merge order — the router dispatches the Merger exactly as the Reviewer-emitted `ferry-merge` does. The Merger stays idempotent: with no open PR for `ferry/<ticket>` it comments and exits.                                                                                                        |
